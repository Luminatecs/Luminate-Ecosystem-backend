...
export class LibraryService {
  ...
  async getAllSchoolData(): Promise<ISchool[]> {
    ...
    try {
      // Try cache first (but don't fail if Redis is unavailable) - COMMENTED OUT FOR NOW
      // try {
      ...
      // Try to cache the data (but don't fail if Redis is unavailable) - COMMENTED OUT FOR NOW
      // try {
  ...
  async searchSchools(searchTerm: string): Promise<ISchool[]> {
    const redisKey = `school_search:${searchTerm}`;
